<?php
//This is a template file for displaying a list of recurring events in a table. These functions are used with the [RECURRING_DROPDOWN] shortcode.
//This is an group of functions for querying all of the events in your databse. 
//This file should be stored in your "/wp-content/uploads/espresso/templates/recurring-dropdown" directory.
...
if (!function_exists('recurring_dropdown_display_all_events')) {
	function recurring_dropdown_display_all_events() {
		recurring_dropdown_get_event_details(array());
	}
}

if (!function_exists('recurring_dropdown_display_categories')) {
	function recurring_dropdown_display_categories($event_category_id=NULL, $css_class=NULL) {
		recurring_dropdown_get_event_details(array('category_identifier' => $event_category_id, 'css_class' => $css_class));
	}
}

//Events Listing - Shows the events on your page.
if (!function_exists('recurring_dropdown_get_event_details')) {

	function recurring_dropdown_get_event_details( $attributes ) {
		//echo $sql; 
		...
		//$sql .= $order_by != 'NULL' ? " ORDER BY " . $order_by . " ASC " : " ";
		...
		/* group recurring events */
		wp_register_script( 'jquery_dropdown', EVENT_ESPRESSO_UPLOAD_URL . 'templates/recurring-dropdown/js/jquery.dropdown.js', array('jquery'), '0.1', TRUE );
		wp_enqueue_script( 'jquery_dropdown' );
		wp_register_style( 'espresso_recurring_dropdown_stylesheet', EVENT_ESPRESSO_UPLOAD_URL . 'templates/recurring-dropdown/css/jquery.dropdown.css');
		...
		foreach ($events_of_same_type as $events_group) {
		    include('recurring_table_display.php');
    }
    ?>
		</table>
		<?php
